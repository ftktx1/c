...
/*   Updated: 2018/01/18 16:57:54 by kshimek          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	**make_array(*char word, int word_len, int word_num)
{
...
char	**ft_split_whitespaces(char *str)
{
	...
	int		**return_array[(sizeof(str) / 2) + 1]; //max number of words possible in a given unknown string
	...
	while (str)
	{
		if (is_whitespace(str[i]) == 0)
		{
			...
			return_arary[k][word_start + j + 1] = '\0'; //[k] word [] word len
		}
		else 
		{
			i++;
